...
static void make_camera(int W,int H,std::vector<float>& o,std::vector<float>& d,std::vector<float>& t0,std::vector<float>& t1){
  o.resize(W*H*3); d.resize(W*H*3); t0.resize(W*H); t1.resize(W*H);
  for(int y=0;y<H;++y){
    for(int x=0;x<W;++x){
      int i=y*W+x;
      o[i*3+0]=0.f; o[i*3+1]=0.f; o[i*3+2]=-2.f;
      float u=(x+0.5f)/W*2.f-1.f;
      float v=(y+0.5f)/H*2.f-1.f;
      float dx=u, dy=-v, dz=1.5f;
      float len=std::sqrt(dx*dx+dy*dy+dz*dz);
      d[i*3+0]=dx/len; d[i*3+1]=dy/len; d[i*3+2]=dz/len;
      t0[i]=0.f; t1[i]=4.f;
    }
  }
}
...
int main(){
  ...
  GridDenseDesc gd{};
  gd.nx=nx; gd.ny=ny; gd.nz=nz;
  gd.bbox_min[0]=-1.f; gd.bbox_min[1]=-1.f; gd.bbox_min[2]=-1.f;
  gd.bbox_max[0]=1.f; gd.bbox_max[1]=1.f; gd.bbox_max[2]=1.f;
  ...
  float Lpos=out_pos.image[(py*W+px)*3+0]+out_pos.image[(py*W+px)*3+1]+out_pos.image[(py*W+px)*3+2];
  ...
  float Lneg=out_neg.image[(py*W+px)*3+0]+out_neg.image[(py*W+px)*3+1]+out_neg.image[(py*W+px)*3+2];
  field_grid_dense_destroy(fp_neg);
  sigma[vid]=old;

  float g_num=(Lpos-Lneg)/(2*eps);
  printf("g_analytic=%g g_numeric=%g relerr=%g\n", g_analytic, g_num, std::abs(g_analytic-g_num)/(std::max(1e-6f,std::abs(g_num))));
